...
import { deleteTestSuite, changeTestSuiteName, postTest, setTestId } from '../../redux/store';

const TestSuiteItem = ({ isEditable, testSuite, testSuiteId, tests }) => {
  ...
  const filteredArray = tests.filter(
    (test) => test.selectedTestSuiteId.testSuiteId === testSuiteId
  );
    ...
    <Box>
      <Box>
        <Box
          ...
          sx={
            !isChangingTestSuiteName && (isEditable || filteredArray.length) > 0
              ? {
                  position: 'relative',
        ...
        >
          ...
          {isEditable && filteredArray.length === 0 && (
            <DeleteIcon
              sx={{
                position: 'absolute',
                top: '50%',
